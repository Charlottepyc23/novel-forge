...
async function extractRoleVisual(ctx, config, project, outputDir, roleName) {
	...
	const raw = parseJsonObject(await complete(ctx, config, {
		system: [
			...
			"  · portrait 立绘：角色设计稿定位，纯白纯色背景，全身完整无裁切，无多余杂物；字段流：年龄感、身高（有依据才写）、发型发色、胡茬、眼眸、面部气质、上身服装分件（含磨损）、下身、鞋、标志物细节。",
...
*/
async function generateRolePromptKit(ctx, config, project, roleName) {
	const role = (project.roles ?? []).find((r) => r.name === roleName);
	if (role === void 0) throw new Error(`角色「${roleName}」不在角色库中`);
	...
	const raw = parseJsonObject(await complete(ctx, config, {
		system: [
			...
			"四类：",
			"1. portrait 立绘：全身/半身正视图，含完整服装与标志物。",
...
function defaultOutputDirFor(bookName) {
	return join("D:\\用户目录\\harness\\novels", bookName.replace(/[\\/:*?"<>|]/g, "").trim().slice(0, 40) || "未命名小说");
}
//#endregion
//#region src/run.ts
/**
