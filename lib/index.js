...
async function extractScenes(ctx, config, project) {
	const system = [
...
async function extractRoleVisual(ctx, config, project, outputDir, roleName, styleId, filterId) {
	...
	const matchesRole = (para) => para.includes(roleName) || searchTokens.some((tok) => para.includes(tok));
	const excerpts = [];
	...
	const raw = parseJsonObject(await complete(ctx, config, {
		system: [
			...
			"【当前视觉风格】（portrait 立绘的「风格」字段必须原样使用，sheet/expressions/details 同样内嵌）：" + styleKeywords(styleId, filterId),
			"硬性要求（依据优先）：",
...
async function generateRolePromptKit(ctx, config, project, roleName, styleId, filterId) {
	...
	const raw = parseJsonObject(await complete(ctx, config, {
		system: [
			...
			"【当前视觉风格】（每类提示词必须内嵌）：" + styleKeywords(styleId, filterId),
			"四类：",
...
async function generateRoleReferenceImage(ctx, config, project, outputDir, name, style = "") {
	...
	if (apiKey === void 0 || apiKey === "" || model === void 0 || model === "") throw new Error("请先在设置中配置豆包 API Key 和生图模型");
	const stylePrompt = getComicStylePrompt(style);
	const prompt = [
		role.imagePrompt.zh,
		role.imagePrompt.en,
		role.imagePrompt.tags.join(", "),
		stylePrompt,
	...
	if (url === void 0 || url === "") throw new Error("豆包生图未返回图片 URL");
	const imgRes = await fetch(url);
	if (!imgRes.ok) throw new Error("下载豆包图片失败");
	const buf = Buffer.from(await imgRes.arrayBuffer());
	const dataUrl = `data:${imgRes.headers.get("content-type") || "image/jpeg"};base64,${buf.toString("base64")}`;
...
function saveChapterStoryboard(project, outputDir, entry) {
	...
	else project.storyboards[idx] = {
		...project.storyboards[idx],
		...entry
	};
	project.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
	saveProject(outputDir, project);
...
async function generateStoryboardTable(ctx, config, project, outputDir, chapterNo, skeleton, styleId, filterId) {
	...
	const scenes = (project.scenes ?? []).filter((s) => body.includes(s.name)).slice(0, 6).map((s) => `${s.name}：${s.summary ?? ""}`);
...
async function generateStoryboardPrompts(ctx, config, project, outputDir, chapterNo, table, styleId, filterId) {
	...
	const rules = (project.visualRules ?? []).map((r) => "- " + r);
	const shotLines = table.shots.map((s) => `[${s.id}] 节拍${s.beatId} · ${s.shot} · ${s.camera} · ${s.duration}s
画面：${s.visual}
台词：${s.line !== "" ? s.line : "（无）"}
光效：${s.light !== "" ? s.light : "（无）"}
承接：${s.prevState} → ${s.nextState}`).join("\n\n");
	const raw = parseJsonObject(await complete(ctx, config, {
		system: [
	...
	const prompts = Array.isArray(raw.prompts) ? raw.prompts.filter((v) => typeof v === "object" && v !== null).map((entry) => ({
		shotId: shotIds.has(entry.shotId) ? entry.shotId : "",
