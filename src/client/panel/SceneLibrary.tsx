  ...
  return (
    <div ref={rootRef} className={css.card} style={{ flex: 1, minHeight: 0 }}>
      ...
      {(project.visualRules ?? []).length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center', margin: '4px 0 8px' }}>
          <span className={css.meta} style={{ fontSize: 11 }}>⚠️ 本书视觉规则（已内嵌到所有提示词）：</span>
      ...
      {draft !== null && (
        <div style={{ border: '1px solid var(--nf-accent)', borderRadius: 10, padding: 10, marginBottom: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <input className={css.input} style={{ fontSize: 12, padding: '4px 8px' }} placeholder="场景名" value={draft.name} onChange={e => { setDraft({ ...draft, name: e.target.value }) }} />
          <input className={css.input} style={{ fontSize: 12, padding: '4px 8px' }} placeholder="一句话定位" value={draft.summary} onChange={e => { setDraft({ ...draft, summary: e.target.value }) }} />
          <div className={css.row} style={{ gap: 6 }}>
            <input className={css.input} style={{ fontSize: 12, padding: '4px 8px', flex: 1 }} placeholder="幕归属（第一幕后场…）" value={draft.act ?? ''} onChange={e => { setDraft({ ...draft, act: e.target.value }) }} />
            <input className={css.input} style={{ fontSize: 12, padding: '4px 8px', flex: 1 }} placeholder="时间光态（雨夜/闭店后…）" value={draft.moment ?? ''} onChange={e => { setDraft({ ...draft, moment: e.target.value }) }} />
          </div>
          <textarea className={css.input} style={{ fontSize: 12, padding: '4px 8px', minHeight: 48 }} placeholder="关键镜头（每行一条：人物动作+情绪+镜头）" value={(draft.beats ?? []).join('\n')} onChange={e => { setDraft({ ...draft, beats: e.target.value.split(/\n+/).map(x => x.trim()).filter(Boolean) }) }} />
          <input className={css.input} style={{ fontSize: 12, padding: '4px 8px' }} placeholder="人物在场状态（含标志物细节）" value={draft.characterState ?? ''} onChange={e => { setDraft({ ...draft, characterState: e.target.value }) }} />
          <textarea className={css.input} style={{ fontSize: 12, padding: '4px 8px', minHeight: 60 }} placeholder="环境构成（每行一项）" value={(draft.elements ?? []).join('\n')} onChange={e => { setDraft({ ...draft, elements: e.target.value.split(/\n+/).map(x => x.trim()).filter(Boolean) }) }} />
          <textarea className={css.input} style={{ fontSize: 12, padding: '4px 8px', minHeight: 60 }} placeholder="中文生图提示词" value={draft.zh} onChange={e => { setDraft({ ...draft, zh: e.target.value }) }} />
          <textarea className={css.input} style={{ fontSize: 12, padding: '4px 8px', minHeight: 60 }} placeholder="英文生图提示词" value={draft.en} onChange={e => { setDraft({ ...draft, en: e.target.value }) }} />
          <div className={css.row} style={{ gap: 6 }}>
      ...
      {scenes.length === 0 && candidates === null ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10 }}>
          {scenes.map(s => {
            ...
            return (
              <div key={s.name} onClick={() => { openDetail(s.name) }} style={{ cursor: 'pointer', border: '1px solid var(--nf-border)', borderRadius: 12, overflow: 'hidden', background: 'var(--nf-bg-inset)', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <b style={{ fontSize: 13 }}>{s.name}</b>{s.styleId !== undefined && <span className={css.badge} style={{ fontSize: 9 }}>🎨 {s.styleId}</span>}
      ...
      {detail !== undefined && createPortal(
        <div style={{ ...portalVars, position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 2147483000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={e => { if (e.target === e.currentTarget) setDetailName(null) }}>
          <div style={{ position: 'relative', background: 'var(--nf-bg)', border: '1px solid var(--nf-border)', borderRadius: 16, padding: 16, width: 'min(860px, 100%)', maxHeight: '88vh', overflow: 'auto', marginTop: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8, marginBottom: 12 }}>
              {(detail.gallery ?? []).map(img => (
                <div key={img.label} style={{ border: '1px solid var(--nf-border)', borderRadius: 10, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <button type="button" className={css.iconButton} style={{ position: 'absolute', top: 2, right: 2, fontSize: 11 }} title="删除" aria-label="删除" onClick={() => { void removeImage(detail.name, img.label) }}>×</button>
                </div>
              ))}
            ...
            <div style={{ border: '1px solid var(--nf-border)', borderRadius: 10, padding: '8px 10px', marginBottom: 10, fontSize: 12 }}>
              <b style={{ fontSize: 12 }}>场景信息</b>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '6px 14px', marginTop: 4, lineHeight: 1.7 }}>
                <div>
                  <b style={{ fontSize: 11 }}>关键镜头</b>
                  <div style={{ marginTop: 2 }}>{(detail.beats ?? []).map((b, i) => <div key={i} style={{ fontSize: 11, color: 'var(--nf-text-2)', marginTop: 2 }}>🎬 {b}</div>)}</div>
                </div>
                <div>
                  {(detail.characterState !== undefined && detail.characterState !== '') && (
                    <div style={{ marginBottom: 6 }}>
                      <b style={{ fontSize: 11 }}>人物状态</b>
                      <div className={css.meta} style={{ marginTop: 2 }}>{detail.characterState}</div>
                    </div>
                  )}
                  <b style={{ fontSize: 11 }}>环境构成</b>
                  <div style={{ marginTop: 2 }}>{(detail.elements ?? []).map(e => <span key={e} style={{ border: '1px solid var(--nf-border)', borderRadius: 999, padding: '1px 8px', fontSize: 11, margin: '0 4px 4px 0', display: 'inline-block' }}>{e}</span>)}</div>
                  <div style={{ marginTop: 6 }}><b style={{ fontSize: 11 }}>色调光影</b> <span className={css.meta}>{(detail.palette ?? []).join('；') || '—'}</span></div>
                  <div style={{ marginTop: 4 }}><b style={{ fontSize: 11 }}>氛围</b> <span className={css.meta}>{(detail.moods ?? []).join('、') || '—'}</span></div>
                  <div style={{ marginTop: 4 }}><b style={{ fontSize: 11 }}>依据</b> <span className={css.meta}>{detail.source || '—'}</span></div>
                </div>
              </div>
            </div>
            ...
            {/* 生图提示词：全局中英 + 复制全部 */}
            <div style={{ marginBottom: 12 }}>
              <div className={css.row} style={{ justifyContent: 'space-between', flexWrap: 'wrap', marginBottom: 6 }}>
              ...
              <div style={{ border: '1px solid var(--nf-border)', borderRadius: 8, padding: 8 }}>
                <div className={css.row} style={{ justifyContent: 'space-between', flexWrap: 'wrap' }}>
                  <b style={{ fontSize: 11 }}>{sceneLang === 'zh' ? '🇨🇳 中文' : '🇬🇧 English'}</b>
                  <button type="button" className={`${css.button} ${css.buttonSmall}`} style={{ padding: '1px 6px', fontSize: 10 }} onClick={() => {
                ...
                <div className={css.meta} style={{ fontSize: 11, lineHeight: 1.6, marginTop: 4, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{sceneLang === 'zh' ? detail.zh : detail.en}</div>
            ...
            <div style={{ borderTop: '1px solid var(--nf-border)', paddingTop: 10, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
              ...
              <input className={css.input} style={{ width: 130, fontSize: 12, padding: '3px 6px' }} placeholder="图集标签" value={uploadLabel} onChange={e => { setUploadLabel(e.target.value) }} />
              <input
                ref={inputRef}
                type="file"
                accept="image/*"
